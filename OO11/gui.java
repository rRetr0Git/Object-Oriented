...
class guiInfo {
	...
	public static int[][] roadmap=new int[6405][6405];
	public static int[][] D  = new int[6405][6405];// 保存从i到j的最小路径值
	public static int[]   Dd = new int[6405];// 保存从i到j的最小路径值
	public static int[]   flow=new int[6405];// 保存从i到j的最小流量值
	...
	public static void pointbfs(int root,int type) {// 单点广度优先搜索
		...
		while (queue.size() > 0) {
			...
			for (int i = 1; i <= 4; i++) {
				...
				if (next >= 0 && next < 6400 &&roadmap[n.NO][next] == 1) {
					if(view[next] == false){
						view[next] = true;
						queue.add(new node(next, n.depth + 1));// 加入遍历队列
						Dd[next]     = n.depth + 1;
					}
					else if(Dd[next] < n.depth){
						flow[n.NO] = Math.min(flow[n.NO],flow[next]+guigv.GetFlow(n.NO/80, n.NO%80, next/80, next%80));
					}
				}
			}
			queue.remove(0);// 退出队列
	...
	public static void pointbfs_simple(int root) {// 单点广度优先搜索
		int[] offset = new int[] { 0, 1, -1, 80, -80 };
		Vector<node> queue = new Vector<node>();
		boolean[] view = new boolean[6405];
		int x = root;// 开始进行单点bfs
...
class guigv {
	...
	public static void AddFlow(int x1,int y1,int x2,int y2){//增加一个道路流量
		//查询之前的流量数量
		int count=0;
		Integer value=guigv.flowmap.get(Key(x1,y1,x2,y2));
	...
	public static void ClearFlow(int x1,int y1,int x2,int y2,int x3,int y3){//清空流量信息
		if(GetFlow(x1,y1,x2,y2)>1){
			guigv.flowmap.put(Key(x1,y1,x2,y2), GetFlow(x1,y1,x2,y2)-1);
			guigv.flowmap.put(Key(x2,y2,x1,y1), GetFlow(x2,y2,x1,y1)-1);
		}
		else if(GetFlow(x1,y1,x2,y2)==1){
			guigv.flowmap.remove(Key(x1,y1,x2,y2));
			guigv.flowmap.remove(Key(x2,y2,x1,y1));
		}
		AddFlow(x2,y2,x3,y3);
