...
//		System.out.println(x1+" "+y1+" "+x2+" "+y2+" "+x3+" "+y3+" ");
	...
	public int getPointOrder(){
		...
		/**
		*@REQUIRES: r!=null;
		*@MODIFIES: this.SRC,this.DST,this.status,this.WaitCount,this.req;
		*@EFFECTS: 
		*		SRC==r.GetSrc();
		*		DST==r.GetDst();
		*		status==TAKING;
	...
	public void stopCount(){
		...
		/**
		*@MODIFIES: this.WaitCount,this.status,this.Loc,this.interval,this.RedLight,this.Blocked;
		*@EFFECTS: 
	...
	public void move(){
		...
		if(this instanceof SuperTaxi){
			Main.roadmap=Main.oldmap;
		}
		else{
			Main.roadmap=Main.map;
		}
		...
		int flow=100;
		if(y>0&&Main.roadmap[x*80+y][x*80+y-1]==1&&guigv.GetFlow(x, y, x, y-1)<flow){
		...
		if(y<79&&Main.roadmap[x*80+y][x*80+y+1]==1&&guigv.GetFlow(x, y, x, y+1)<=flow){
			if(guigv.GetFlow(x, y, x, y+1)<flow){
				Dir.clear();
				Dir.add("RIGHT");
				flow=guigv.GetFlow(x, y, x, y+1);
		...
		if(x>0&&Main.roadmap[x*80+y][(x-1)*80+y]==1&&guigv.GetFlow(x, y, x-1, y)<=flow){
		...
		if(x<79&&Main.roadmap[x*80+y][(x+1)*80+y]==1&&guigv.GetFlow(x, y, x+1, y)<=flow){
			if(guigv.GetFlow(x, y, x+1, y)<flow){
				Dir.clear();
				Dir.add("DOWN");
				flow=guigv.GetFlow(x, y, x+1, y);
	...
	public void move(Point p){
		...
		if(this instanceof SuperTaxi){
			Main.roadmap=Main.oldmap;
			guiInfo.pointbfs(dx*80+dy,1);
		}
		else{
			Main.roadmap=Main.map;
			guiInfo.pointbfs(dx*80+dy,0);
		}
		...
		int flow=100000;
		ArrayList<String> dir = new ArrayList<String>();
		if(ly>0&&Main.roadmap[lx*80+ly][lx*80+ly-1]==1&&guiInfo.Dd[lx*80+ly-1]==(dis-1)&&guiInfo.flow[lx*80+ly-1]<=flow){
		...
		else if(ly<79&&Main.roadmap[lx*80+ly][lx*80+ly+1]==1&&guiInfo.Dd[lx*80+ly+1]==(dis-1)&&guiInfo.flow[lx*80+ly+1]<=flow){
			if(guiInfo.flow[lx*80+ly+1]+guigv.GetFlow(lx, ly, lx, ly+1)<flow){
				dir.clear();
				dir.add("RIGHT");
				flow=guiInfo.flow[lx*80+ly+1];
		...
		else if(lx>0&&Main.roadmap[lx*80+ly][(lx-1)*80+ly]==1&&guiInfo.Dd[(lx-1)*80+ly]==(dis-1)&&guiInfo.flow[(lx-1)*80+ly]<=flow){
		...
		else if(lx<79&&Main.roadmap[lx*80+ly][(lx+1)*80+ly]==1&&guiInfo.Dd[(lx+1)*80+ly]==(dis-1)&&guiInfo.flow[(lx+1)*80+ly]<=flow){
			if(guiInfo.flow[(lx+1)*80+ly]+guigv.GetFlow(lx, ly, lx+1, ly)<flow){
				dir.clear();
				dir.add("DOWN");
				flow=guiInfo.flow[(lx+1)*80+ly];
