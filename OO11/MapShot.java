...
public class MapShot {
	/**
	 *@OVERVIEW: MapShot mainly contains a 2-dimension array which store the taxi'positon and other information
	 *			the Scheduler and Taxi Squad use this array to arrange the movement and order-taking
	 *			when it's method is used, something will be changed in the array;
	 */
	private int[][][] Shot;
		/**
		*@MODIFIES: this.Shot;
	...
	public boolean repOK(){
		if(this.Shot==null)return false;
		for(int i=0;i<80;i++){
			for(int j=0;j<80;j++){
				for(int k=0;k<100;k++){
					if(this.Shot[i][j][k]!=-1){
						for(int l=k+1;l<100;l++){
							if(this.Shot[i][j][l]!=-1){
								if(this.Shot[i][j][k]==this.Shot[i][j][l])return false;
							}
						}
					}
				}
