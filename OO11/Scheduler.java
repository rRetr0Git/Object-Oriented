...
public class Scheduler extends Thread{
	/**
	 *@OVERVIEW:Scheduler contains every resources to dispatch requests 
	 *			such as MapShot,RequestList,information of taxis
	 *			it can dispatch every due request and select the appropriate taxi to take it
	 *			this thread is always wait to be notified and do dispatching;
	 */
		...
		for(int i=0;i<this.RL.size();i++){
			...
			if(due(this.RL.get(i))){
				...
				if(id!=-1){
					...
					if(id<30){
						((SuperTaxi)this.Squad[id]).ReqList.add(this.RL.get(i));
					}
	...
	public int Select(Request r){
		...
		for(int i=0;i<r.getList().size();i++){
			...
			if(status==2&&tcredit>credit){
				if(begin==1){
					if(id<30){
						guiInfo.pointbfs(((int)r.GetSrc().getX())*80+(int)r.GetSrc().getY(),1);
					}
					else{
						guiInfo.pointbfs(((int)r.GetSrc().getX())*80+(int)r.GetSrc().getY(),0);
					}
					begin=0;
				}
	...
	public boolean repOK(){
		...
		if(!this.RL.repOK())return false;
		if(this.TS==null)return false;
		if(!this.TS.repOK())return false;
